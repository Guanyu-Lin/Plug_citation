...
def init_all(config, gpu_list, checkpoint, mode, *args, **params):
    ...
    optimizer = init_optimizer(model.model.doc_plug_mapper, config, *args, **params)
    # import pdb
    # pdb.set_trace()
    ...
    elif lrsche_type == "cosine":
        lr_scheduler = bmt.lr_scheduler.Cosine(optimizer, start_lr=config.getfloat('train', 'learning_rate'), warmup_iter=config.getint('train', 'warmup_steps'), end_iter=config.getint('train', 'training_steps'))
