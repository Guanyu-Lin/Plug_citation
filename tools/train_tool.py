...
def train(parameters, config, gpu_list, do_test=False, only_eval=False):
	...
	for epoch_num in range(0, epoch):
		model.model.doc_plug_mapper.train()
		start_time = timer()
...
# break
				if (step + 1) == total_len and (epoch_num == 9 or epoch_num == 0):
				...
				if step % output_time == 0:
					...
					print_rank("step: %d/%d" % (step + 1, total_len))
					print_rank("loss: %.3lf" % (total_loss / (step + 1)))
				...
				if (step + 1) % grad_accumulate == 0 and valid_mode == 'step' and int((step + 1) / grad_accumulate) % step_epoch == 0:
					...
					if not no_valid:
						with torch.no_grad():
							em, hit, length = valid(model, parameters["valid_dataset"], current_epoch, config, gpu_list, output_function)
							print_rank("EM: %.3lf" % (em))
							print_rank("hit: %.3lf" % (hit))
							print_rank("length: %.3lf" % (length))
							if em > BEST_EM:
								BEST_EM = em
			...
			# 			"%.3lf" % (total_loss / (step + 1)), None, config)
		...
		if current_epoch % test_time == 0:
			with torch.no_grad():
				em, hit, length = valid(model, parameters["valid_dataset"], current_epoch+2, config, gpu_list, output_function)
				print_rank("EM: %.3lf" % (em))
				print_rank("hit: %.3lf" % (hit))
				print_rank("length: %.3lf" % (length))
				if em > BEST_EM:
