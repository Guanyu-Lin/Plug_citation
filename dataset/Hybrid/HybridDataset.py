...
# from utils import *
from dataset.Hybrid.utils import *
...
class HybridDataset(Dataset):
    def __init__(self, config, mode, encoding="utf8", *args, **params):
        ...
        eval_data = list(json_lines.reader(open(config.get("data", "%s_data_path" % mode))))
        ...
        for idx, eval_item in enumerate(tqdm(eval_data)):
           # question = head_prompt + make_demo(
            ...
            doc_list = get_shorter_text(eval_item, eval_item["passage"], self.ndoc, self.use_shorter) if self.use_shorter is not None else eval_item["passage"][:self.ndoc]
            if not self.retrieve_in_all_docs:
                ...
                # import pdb
                # pdb.set_trace()
                eval_data[idx]['passage'] = doc_list
