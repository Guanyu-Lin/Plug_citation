...
class Seq2Seq(nn.Module):
	...
	def forward(self, data, config, gpu_list, pre_result, mode, save_score=None):
		...
		if pre_result is None:
			pre_result = [{"answer": ans, "output": out, "question":  ques, "qa_pairs":  pair} for ans, out, ques, pair in zip(data["answers"], result_text, data["pure_question"],data["qa_pairs"] )]
		else:
			pre_result.extend([{"answer": ans, "output": out, "question":  ques, "qa_pairs":  pair} for ans, out, ques, pair in zip(data["answers"], result_text, data["pure_question"],data["qa_pairs"] )])
