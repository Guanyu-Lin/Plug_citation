...
class SelfAttentionBlock(torch.nn.Module):
    ...
    def forward(self,
        ...
        if prefix is not None:
            #seq = torch.cat([self.layernorm_before_attention(prefix), x], dim=1).contiguous()
            if not self.sparse_attention:
                x = self.self_attention(x, x, attention_mask, position_bias, parameters=self.layernorm_before_attention(prefix), use_cache=use_cache, past_key_value=past_key_value, save_score=save_score)
            else:
                #no position bias for sparse attention
                x = self.self_attention(x, attention_mask, position_bias)
        else:
            if not self.sparse_attention:
                x = self.self_attention(x, x, attention_mask, position_bias, parameters=None, use_cache=use_cache, past_key_value=past_key_value,save_score=save_score)
            else:
                #no position bias for sparse attention
                x = self.self_attention(x, attention_mask, position_bias)
...
class CrossAttentionBlock(torch.nn.Module):
    ...
    def forward(self,
        ...
        if use_cache:
            x, current_key_value, score = x
        else:
            current_key_value, score = None
        ...
        if use_cache:
            return hidden_states, current_key_value, score
        else:
...
class TransformerPlugBlock(torch.nn.Module):
    ...
    def forward(self,
        ...
        if save_score:
            # self.cur_layer = self.layer_no
            save_score = save_score + "_layer_" + str(self.layer_no)
        ...
        if pfx is not None:
            self_attention_mask = torch.concat([self_attention_mask, prefix_mask], dim=-1)
        current_key_value = None
