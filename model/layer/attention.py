...
#				pdb.set_trace()
				...
				if parameters is not None:
						len_k_prefix = parameters.size(1)
						h_k_prefix = self.project_k(parameters) 
						h_v_prefix = self.project_v(parameters)
						h_k_prefix = h_k_prefix.view(batch_size, len_k_prefix, self.num_heads_kv, self.dim_head).permute(0, 2, 1, 3)
						h_v_prefix = h_v_prefix.view(batch_size, len_k_prefix, self.num_heads_kv, self.dim_head).permute(0, 2, 1, 3)
						if self.num_head_groups != 1 and self.num_heads_kv != 1:
								h_k_prefix = h_k_prefix[:, :, None, :, :].expand(batch_size, self.num_heads_kv, self.num_head_groups, len_k_prefix, self.dim_head).reshape(batch_size, self.num_heads, len_k_prefix, self.dim_head)
								h_v_prefix = h_v_prefix[:, :, None, :, :].expand(batch_size, self.num_heads_kv, self.num_head_groups, len_k_prefix, self.dim_head).reshape(batch_size, self.num_heads, len_k_prefix, self.dim_head)
						h_k_prefix = h_k_prefix.contiguous()
						h_v_prefix = h_v_prefix.contiguous()
						h_k = torch.cat([h_k, h_k_prefix], dim=-2)
						h_v = torch.cat([h_v, h_v_prefix], dim=-2)
						# import pdb
						# pdb.set_trace()
						len_k = h_k.size(-2)
				...
				# print("in attention", attention_mask.shape)
				# print("q", len_q)
				# print("k", len_k)
				...
				if save_score is not None and 'plug' not in save_score and parameters is not None:
						matrix = mask_score[0,0]

						matrix = torch.where(torch.isinf(matrix),torch.full_like(matrix, 0),matrix)

						sns.heatmap(matrix.detach().cpu().numpy().tolist(), center=0,)
						plt.ylabel('query',fontsize=13)
						plt.xlabel('key',fontsize=13)
						plt.tight_layout()
						plt.savefig("plot_fig/" + save_score+"_mask.png")
						plt.clf()
